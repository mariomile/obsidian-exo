...
It never creates notes (except a daily inbox, `<memory folder>/memory/inbox/YYYY-MM-DD.md`, for facts with no obvious home), never touches hidden folders, `Input/` or Readwise imports, folders you excluded in Obsidian, or the agent's own kernel files, and writes at most 8 lines per chat.

**Before each message** Exo searches the vault and your chats from the last 60 days for what you're asking about, and quietly adds the top matches as background (a *Recalled N* row under your message shows exactly what). Short messages and slash commands are skipped. The agent can also read your recent conversations on demand with the `recent_chats` tool.

**Review and undo.** Each harvest shows a notice (*Exo remembered 2 things from "Pricing"*), and the Memory tab of the Capabilities hub lists recent memory writes line by line, each with an **Undo** button. In a git vault every harvest is one commit, `exo: memory harvest: <n> scritture (<chat title>)`, touching only those notes; undo is a `git revert` (`exo: memory revert: <sha>`). Without git, undo restores the snapshot taken before the write. Either way Exo refuses to undo if you edited those notes since. You can also run **Exo Agent: Undo last memory write**, or just ask Exo to undo it.
