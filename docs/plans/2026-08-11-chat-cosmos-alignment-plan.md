...
- **Ratchet slack is zero (still true after the Phase 6 review pass below).** `design-contract.json` sits at exactly the current counts (hex 49, `!important` 6). The next commit that adds either fails the build, which is the point, but nobody has taken a pass at lowering them yet.

### 2026-08-12: Phase 6, the review pass it never got

**Task.** The open item above ("Phase 6 exited review UNRESOLVED, worth one fresh pass") was worked as an adversarial review of `9326143`, `78cf6d7`, `2673e1b`. Seven findings, six real. Every fix landed test-first, and the greps that pinned the band's wiring were replaced with behavioural tests against a DOM stub rich enough to host the real element.

| Finding | Verdict | What changed |
| --- | --- | --- |
| Turn end swallowed a stretch no reveal had reported | real | `noteTurnEnd` takes `turnFrom` (where the ended turn's own messages start) and `core/reentry.ts` `readIndexAfterTurn` refuses to jump over unreported work behind it. Opening the sidebar mid-run no longer costs you the line. |
| The band was one-shot per transcript | real | `reenterActive` gated on mere presence of `.mva-reentry`, and nothing ever removes it. Now gated on `bandIsStale`: a painted band is left alone only while the position is still caught up, so work landing behind it repaints the line. |
| Transcript resets left a stale read position | real | `noteTranscriptReset` on `newSessionInTab`; the two rewinds carry it through `cutTranscriptAfter` / `cutTranscriptFrom`. A stored position that outlived its messages used to clamp to "caught up" forever. |
| Rewinds indexed turns by DOM position | real | `turnMessageIndex(turnEl)` reads the `data-msg` stamp; only element REMOVAL still walks the DOM (the phantom turns have to go too). The `appendChild` fallback under a missed anchor is gone: a band with no anchor now refuses to paint and leaves the position alone, because a miss was a confidently wrong placement. |
| "N questions waiting" could only ever count cancelled cards | real | The slot is gone. A `plan` / `ask` segment reaches `messages` only at turn end, and that teardown cancels whatever card was open, so the count was always dead cards; while a card is genuinely open the chat is streaming and the band cannot paint at all. `REENTRY_SLOTS` is now `steps, files`. This overrides the Phase 6 criterion above that listed three slots. |
| `restore()` revealed with no visibility gate | real, and reachable | `convo-bridge.chatView()` calls `loadIfDeferred()`, so background work materialises the view (and runs `restore`) inside a collapsed sidebar. Deferred views do not make it moot. Boot and `switchTo` now both go through `reenterActive`, i.e. through `isShown()`. |
| The wiring tests did not pin behaviour | real | `revealReentry` / `reenterActive` / the cut helpers are now driven directly. What is left of the source-reading tests is a short list of exact call sites in `view.ts`, which no test can reach without a mounted Obsidian. |

**Cost of the size ratchet.** `view.ts` sat at exactly 6600. The fixes were written to land at net zero lines there: the transcript cut moved into `ui/reentry.ts` (`cutTranscriptAfter` / `cutTranscriptFrom`), and the reasoning lives in `core/reentry.ts` and `ui/reentry.ts` with one-line pointers at the call sites.

**Verification.** `pnpm lint` 0 errors / 8 pre-existing warnings; `pnpm test` 2583 tests, 156 files, 0 failures; `pnpm build` green with `design contract OK (styles.css: hex 49/49, !important 6/6)`; `src/view.ts` 6600 lines, at the ceiling and not over it.
