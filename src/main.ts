...
export default class ExoPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    // Orchestration board icon — Huge Icons (hugeicons.com, free/MIT,
    // Stroke Rounded, 24x24 grid). addIcon's viewBox is fixed at 100x100, so
    // scale by 100/24 to fill it correctly.
    addIcon(
      BOARD_ICON,
    ...
    // Cockpit icon — Huge Icons dashboard-speed (gauge with needle).
    addIcon(
      COCKPIT_ICON,
      '<g transform="scale(4.166667)" fill="none" stroke="currentColor" stroke-width="1.5">' +
   ...
   */
  async runAgent(agent: AgentDef, reason: string, runKey?: string): Promise<boolean> {
    ...
    try {
      ...
      const result = await runHeadlessPlaybook(this.app, this.settings, buildAgentRunPrompt(agent, reason), { write });
      const path = await writeReport(this.app, name, result, this.paths.reports);
